cat > /mnt/user-data/outputs/page_optimized.tsx << 'ENDOFFILE'
...
export default function CafeDidarApp() {
  ...
  useEffect(() => { if (menu.length > 0) localStorage.setItem('cafe_menu', JSON.stringify(menu)); }, [menu]);
  useEffect(() => { localStorage.setItem('cafe_feedback', JSON.stringify(feedback)); }, [feedback]);
  useEffect(() => { localStorage.setItem('cafe_gallery', JSON.stringify(gallery)); }, [gallery]);
    ...
    <div className="flex flex-col min-h-screen max-w-[500px] mx-auto relative overflow-x-hidden bg-[#1C0F0A] font-body" dir="rtl">
      ...
      <main className="mt-[70px] mb-[70px] flex-1 overflow-y-auto">
        <div className="animate-fade-in">
          ...
          {currentView === 'ADMIN_DASHBOARD' && <AdminDashboard menu={menu} setMenu={setMenu} feedback={feedback} gallery={gallery} setGallery={setGallery} />}
        </div>
      </main>
...
function CategoryTab({ active, label, onClick }: { active: boolean; label: string; onClick: () => void }) {
  return (
    <button onClick={onClick} className={cn('flex-1 py-2.5 text-[11px] font-black rounded-xl mx-0.5 transition-all duration-200 active:scale-95', active ? 'bg-[#D4A853] text-[#1C0F0A] shadow-lg shadow-[#D4A853]/20' : 'text-[#5A4A44] hover:text-[#A89B95]')}>
      {label}
    </button>
  );
}
        ...
        <div className="space-y-3">
          ...
          <div className="mt-6 bg-[#2A1810] p-5 rounded-2xl border border-[#D4A853]/20">
            <div className="flex justify-between items-center mb-5">
              ...
              <span className="text-2xl font-black text-[#D4A853]">
                {(total / 1000).toLocaleString()} <span className="text-xs font-bold">تومان</span>
              </span>
            </div>
...
function AdminMenuManager({ menu, setMenu }: any) {
  ...
  const handleSave = () => {
    ...
    if (editingId) {
      setMenu(menu.map((m: any) => m.id === editingId ? { ...m, ...form } : m));
    } else {
      setMenu([{ ...form, id: Date.now().toString() } as MenuItem, ...menu]);
    }
    setIsOpen(false);
    setEditingId(null);
    setForm(emptyForm);
  ...
  return (
    <div className="space-y-3">
      <Button onClick={() => { setIsOpen(!isOpen); setEditingId(null); setForm(emptyForm); }}
      ...
      <div className="space-y-2">
        {menu.map((m: any) => (
          <div key={m.id} className="bg-[#2A1810] p-3 rounded-xl flex items-center gap-3 border border-[#3D2B24]">
            ...
            <button onClick={() => setMenu(menu.filter((x: any) => x.id !== m.id))} className="text-red-500/30 hover:text-red-500 transition-colors p-1">
              <Trash2 size={14} />
            </button>
          </div>
        ))}
...
function AdminGalleryManager({ gallery, setGallery }: any) {
  const handleUpload = (e: any) => {
    ...
    if (!file) return;
    const reader = new FileReader();
    reader.onloadend = () => setGallery((prev: any[]) => [{ id: Date.now(), url: reader.result as string }, ...prev]);
    reader.readAsDataURL(file);
  };
      ...
      <div className="grid grid-cols-3 gap-2">
        {gallery.map((img: any) => (
          <div key={img.id} className="relative aspect-square rounded-lg overflow-hidden group">
            <img src={img.url} className="w-full h-full object-cover" alt="" />
            <button onClick={() => setGallery((prev: any[]) => prev.filter((x: any) => x.id !== img.id))}
